...
// task("maketoken", "Make 2 dummy tokens for use in tests", require("./token.js")) 
//   .addParam("symbol", "The tokens symbol", "TK")
//     .addParam("name", "The tokens name", "Token")
//     .addParam("mintamount", "amount to premint", "10000000")
...
task("maker:serve", "BluntBar serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
.setAction(async function ({ a, b }, { ethers: { getNamedSigner } }, runSuper) {
  const maker = await ethers.getContract("BluntRoller")

  await (await maker.connect(await getNamedSigner("dev")).convert(a, b, { gasLimitgasLimit: 2198000 })).wait()
});
