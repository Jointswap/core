...
describe("BluntBar", function () {
  before(async function () {
    this.JointToken = await ethers.getContractFactory("JointToken")
    this.BluntBar = await ethers.getContractFactory("BluntBar")
  ...
  beforeEach(async function () {
    this.sushi = await this.JointToken.deploy()
    this.bar = await this.BluntBar.deploy(this.sushi.address)
  ...
  it("should work with more than one participant", async function () {
    ...
    // BluntBar get 20 more SUSHIs from an external source.
